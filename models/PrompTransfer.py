...
class Model(nn.Module):
    ...
    def __init__(self, configs, patch_len=16, stride=8):
        ...
        self.soft_prompt_cross_attn_layer = ReprogrammingLayer(
            configs.d_model, configs.n_heads, self.d_ff, self.d_llm)
    ...
    def forecast(self, x_enc, x_mark_enc, x_dec, x_mark_dec):
        ...
        enc_out = self.reprogramming_layer(
            x_enc, source_embeddings, source_embeddings)
        soft_prompt = self.soft_prompt_cross_attn_layer(x_enc, source_embeddings, source_embeddings)
        soft_prompt = self.llm_model(inputs_embeds=soft_prompt).last_hidden_state
        llama_enc_out = torch.cat([prompt_embeddings, enc_out], dim=1)
        llama_enc_out = torch.cat([llama_enc_out,soft_prompt], dim=1)
